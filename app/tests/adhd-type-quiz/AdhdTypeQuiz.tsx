...
function getResult(answers: number[]) {
  const iScore = questions.filter(q => q.type === "I").map((q) => answers[questions.indexOf(q)] ?? 0).reduce((a, b) => a + b, 0);
  const hScore = questions.filter(q => q.type === "H").map((q) => answers[questions.indexOf(q)] ?? 0).reduce((a, b) => a + b, 0);
